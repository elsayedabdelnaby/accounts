...
class CourseBranch {
    ...
    public function add($course_branch) { // take student(associative array) array hold all data you will insert it
        ...
        $stmt = $this->conn->prepare('INSERT INTO meds_course_branch (course_id, branch_id, start_date, end_date, lectures_number, price, currency_id, created_by) 
        VALUES (:course_id, :branch_id, :start_date, :end_date, :lectures_number, :price, :currency_id, :created_by)');
    ...
    public function get($conditions) {
        $query = 'SELECT meds_course_branch.id, meds_course_branch.course_id, meds_course_branch.branch_id, meds_course_branch.start_date, '
                ...
                . ' LEFT JOIN meds_users AS updators ON meds_course_branch.updated_by = updators.id'
                . ' INNER JOIN meds_branches ON meds_course_branch.branch_id = meds_branches.id'
                . ' INNER JOIN meds_courses ON meds_course_branch.course_id = meds_courses.id WHERE ';
        if (!empty($conditions)) {
            $count = 0;
    ...
    public function update($course_branch) {
        $query = 'UPDATE meds_course_branch SET course_id = ?, branch_id = ?, start_date = ?, end_date = ?, lectures_number = ?, price = ?, updated_by = ? WHERE id = ?';
        $stmt = $this->conn->prepare($query);
        ...
        $stmt->bindParam(8, $course_branch['id'], PDO::PARAM_INT);
        if ($stmt->execute()) {
